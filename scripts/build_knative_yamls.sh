docker build -f ./workflows/knative_yaml_builder/Dockerfile ./workflows -t yaml-builder:latest
tmpdir="$(mktemp -d)"
chmod 777 "$tmpdir"
docker run --rm -v "$(pwd)"/workflows/image/airflow-dags:/dag_import -v "$tmpdir":/output -v "$(pwd)"/workflows/knative_yaml_builder/knative_service_template.yaml:/knative_service_template.yaml yaml-builder:latest

# 清理旧的 YAML 文件，避免重复追加
rm -rf ./workflows/knative_yamls
mkdir -p ./workflows/knative_yamls

for dag_dir in "$tmpdir"/*; do
	cp -r "$dag_dir" ./workflows/knative_yamls/
done

find ./workflows/knative_yamls -name "*.yaml" -type f | while read -r file; do
	sed -i -e '$a\' "$file"
    echo "        autoscaling.knative.dev/minScale: \"1\"" >> "$file"
    # echo "        autoscaling.knative.dev/minScale: \"0\"" >> "$file"
    # echo "        autoscaling.knative.dev/scale-to-zero-grace-period: \"200s\"" >> "$file"
    # echo "        autoscaling.knative.dev/scale-to-zero-pod-retention-period: \"200s\"" >> "$file"
    # echo "        autoscaling.knative.dev/stable-window: \"200s\"" >> "$file"
    echo "Modified $file"
done

echo "All YAML files have been processed."