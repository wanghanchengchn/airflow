...
kubectl -n airflow logs "$scheduler" scheduler --since-time="$START_TIME" | grep WHC_E2E_LATENCY > "$log_dir"/log_whc_e2e_latency.log
...
do
    ...
    kubectl -n airflow logs "$worker" user-container --since-time="$START_TIME" | sed -r "s/\x1B\[([0-9]{1,3}(;[0-9]{1,3})*)?[mGK]//g" > "$log_dir"/log_worker${i}.log
    worker_logs+=" $log_dir/log_worker${i}.log"
done

echo $worker_logs

python3 scripts/10_merge_logs.py "$log_dir"/log_scheduler.log $worker_logs -o "$log_dir"/merged_output.log
...
cat "$log_dir"/log_whc_e2e_latency.log | python3 scripts/7_process_e2e_latency.py $REPETITION 2         # 有几组？每组有2行

echo " "
echo "9_process_e2e_breakdown.py"
grep WHC_E2E_BREAKDOWN "$log_dir"/merged_output.log > "$log_dir"/log_whc_e2e_breakdown.log
cat "$log_dir"/log_whc_e2e_breakdown.log | python3 scripts/9_process_e2e_breakdown.py $REPETITION $LINES_PER_GROUP  # 有几组？每组有2 + 4 * d行